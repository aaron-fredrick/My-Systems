(function(){
  ...
  function draw(){ctx.clearRect(0,0,w,h);const cx=w/2,cy=h/2+8,r=Math.min(w,h)*R;const grd=ctx.createRadialGradient(cx-r*.25,cy-r*.3,r*.05,cx,cy,r*1.05);grd.addColorStop(0,'rgba(91,167,255,.055)');grd.addColorStop(.72,'rgba(91,167,255,.014)');grd.addColorStop(1,'rgba(91,167,255,0)');ctx.fillStyle=grd;ctx.beginPath();ctx.arc(cx,cy,r*1.04,0,Math.PI*2);ctx.fill();for(let lat=-90;lat<=90;lat+=dotStep)for(let lon=-180;lon<180;lon+=dotStep){const p=project(lat,lon);if(p.z<-.04)continue;const q=point(p,cx,cy,r),alpha=.08+.18*Math.max(0,p.z);ctx.fillStyle=`rgba(91,167,255,${alpha})`;ctx.beginPath();ctx.arc(q.x,q.y,Math.max(.65,1.15*p.z),0,Math.PI*2);ctx.fill()}const projected=nodes.map((n,i)=>({node:n,index:i,p:project(n.lat,n.lon)})).map(o=>({...o,q:point(o.p,cx,cy,r)}));ctx.lineWidth=1;connections.forEach(([a,b],ci)=>{const A=projected[a],B=projected[b];if(A.p.z<-.05&&B.p.z<-.05)return;const near=hoverNode===a||hoverNode===b,visibility=Math.max(.05,(A.p.z+B.p.z+2)/4);ctx.strokeStyle=near?`rgba(91,167,255,${.38*visibility})`:`rgba(91,167,255,${.10*visibility})`;ctx.lineWidth=near?1.5:1;ctx.beginPath();ctx.moveTo(A.q.x,A.q.y);ctx.lineTo(B.q.x,B.q.y);ctx.stroke();if(!reduce){const t=(performance.now()/3000+ci*.17)%1,px=A.q.x+(B.q.x-A.q.x)*t,py=A.q.y+(B.q.y-A.q.y)*t;ctx.fillStyle=`rgba(91,167,255,${(near?.65:.24)*visibility})`;ctx.beginPath();ctx.arc(px,py,near?1.9:1.2,0,Math.PI*2);ctx.fill()}});if(hoverNode>=0){const o=projected[hoverNode];if(o&&o.p.z>.03){ctx.strokeStyle='rgba(91,167,255,.34)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(o.q.x,o.q.y,12+Math.sin(performance.now()/190)*2.5,0,Math.PI*2);ctx.stroke()}}projected.sort((a,b)=>a.p.z-b.p.z).forEach(o=>{const front=o.p.z>-.02,active=hoverNode===o.index,size=o.node.known?4.2:2.7;ctx.globalAlpha=front?(active?1:.82):.28;if(active||o.node.known){ctx.shadowBlur=active?17:8;ctx.shadowColor='rgba(91,167,255,.65)'}else ctx.shadowBlur=0;ctx.fillStyle='#5ba7ff';ctx.beginPath();ctx.arc(o.q.x,o.q.y,size,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;if(o.node.known&&front){ctx.font='500 11px "DM Mono",monospace';ctx.fillStyle=active?'#f3f6f8':'#aeb9c4';ctx.textAlign=o.q.x<cx?'right':'left';ctx.textBaseline='middle';ctx.fillText(o.node.name,o.q.x+(o.q.x<cx?-11:11),o.q.y)}});ctx.globalAlpha=1;if(!reduce)requestAnimationFrame(draw)}
