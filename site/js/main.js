(function(){
  ...
  let w=0,h=0,dpr=1,rotX=-.08,rotY=-.18,targetX=rotX,targetY=rotY,drag=false,lastX=0,lastY=0,hoverNode=null;
  const R=.31,dotStep=9;
  ...
  function draw(){
    ...
    const grd=ctx.createRadialGradient(cx-r*.25,cy-r*.3,r*.05,cx,cy,r*1.05);grd.addColorStop(0,'rgba(91,167,255,.045)');grd.addColorStop(.72,'rgba(91,167,255,.012)');grd.addColorStop(1,'rgba(91,167,255,0)');ctx.fillStyle=grd;ctx.beginPath();ctx.arc(cx,cy,r*1.04,0,Math.PI*2);ctx.fill();
    ...
    ctx.lineWidth=1;connections.forEach(([a,b])=>{const A=projected[a],B=projected[b];if(A.p.z<-.05&&B.p.z<-.05)return;const visibility=Math.max(.05,(A.p.z+B.p.z+2)/4);ctx.strokeStyle=`rgba(91,167,255,${.10*visibility})`;ctx.beginPath();ctx.moveTo(A.q.x,A.q.y);ctx.lineTo(B.q.x,B.q.y);ctx.stroke();if(!reduce){const t=(performance.now()/2600)%1,px=A.q.x+(B.q.x-A.q.x)*t,py=A.q.y+(B.q.y-A.q.y)*t;ctx.fillStyle=`rgba(91,167,255,${.25*visibility})`;ctx.beginPath();ctx.arc(px,py,1.3,0,Math.PI*2);ctx.fill()}});
    projected.sort((a,b)=>a.p.z-b.p.z).forEach(o=>{const front=o.p.z>-.02,active=hoverNode===o.index,size=o.node.known?3.8:2.6;ctx.globalAlpha=front?(active?1:.82):.28;if(active||o.node.known){ctx.shadowBlur=active?18:10;ctx.shadowColor='rgba(91,167,255,.7)'}else ctx.shadowBlur=0;ctx.fillStyle='#5ba7ff';ctx.beginPath();ctx.arc(o.q.x,o.q.y,size,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;if(o.node.known&&front){ctx.font='500 11px "DM Mono",monospace';ctx.fillStyle=active?'#f3f6f8':'#aeb9c4';ctx.textAlign=o.q.x<cx?'right':'left';ctx.textBaseline='middle';ctx.fillText(o.node.name,o.q.x+(o.q.x<cx?-10:10),o.q.y)}});ctx.globalAlpha=1;if(!reduce)requestAnimationFrame(draw);
  }
  function tick(){if(!drag)targetY+=(Math.abs(targetY-rotY)<.01?.0009:0);rotY+=(targetY-rotY)*.08;rotX+=(targetX-rotX)*.08;if(reduce)draw();else requestAnimationFrame(tick)}
  function pointerPos(e){const rect=canvas.getBoundingClientRect();return{x:e.clientX-rect.left,y:e.clientY-rect.top}}
  function findNode(x,y){const cx=w/2,cy=h/2+8,r=Math.min(w,h)*R;let best=-1,dist=18;nodes.forEach((n,i)=>{const p=project(n.lat,n.lon);if(p.z<.03)return;const q=point(p,cx,cy,r),d=Math.hypot(q.x-x,q.y-y);if(d<dist){dist=d;best=i}});return best}
  canvas.addEventListener('pointerdown',e=>{drag=true;lastX=e.clientX;lastY=e.clientY;canvas.setPointerCapture(e.pointerId);globe.classList.add('interacted')});
  canvas.addEventListener('pointermove',e=>{if(drag){const dx=e.clientX-lastX,dy=e.clientY-lastY;targetY+=dx*.008;targetX=Math.max(-1.15,Math.min(1.15,targetX+dy*.008));lastX=e.clientX;lastY=e.clientY}const p=pointerPos(e);hoverNode=findNode(p.x,p.y)});
  canvas.addEventListener('pointerup',e=>{drag=false;try{canvas.releasePointerCapture(e.pointerId)}catch(_){} });canvas.addEventListener('pointercancel',()=>drag=false);canvas.addEventListener('pointerleave',()=>{if(!drag)hoverNode=null});
  draw();tick();
})();