...
export function initScrollMotion() {
  ...
  if (window.ScrollTrigger) {
    ...
    gsap.utils.toArray('.reveal').forEach((el,index)=>{
      ...
      gsap.fromTo(el,{opacity:0,y:el.classList.contains('principle')?38:28},{opacity:1,y:0,duration:.8,delay:Math.min(index*.055,.3),ease:'power3.out',scrollTrigger:{trigger:el,start:'top 88%',once:true}});
    });
    ...
    document.querySelectorAll('.section-label').forEach(label=>{
      ...
      gsap.fromTo(label,{x:-12,opacity:.25},{x:0,opacity:1,duration:.7,scrollTrigger:{trigger:label,start:'top 90%',once:true}});
    });
    ...
    if(cta){
      gsap.fromTo('.cta-content',{y:70,opacity:.65},{y:0,opacity:1,ease:'none',scrollTrigger:{trigger:cta,start:'top 90%',end:'center center',scrub:.9}});
