...
export function initGlobe() {
  ...
  const nodes = [
    ...
    { name: '', lat: 58, lon: -92 }, { name: '', lat: 8, lon: -118 },
  ...
  let drag=false,lastX=0,lastY=0,hover=-1,vx=0,vy=0,lastInteraction=0,pointerX=0,pointerY=0;
  ...
  function draw(){
    ctx.clearRect(0,0,w,h);const cx=w/2+pointerX*7,cy=h/2+8+pointerY*5,r=Math.min(w,h)*radius;
    ...
    for(let lat=-90;lat<=90;lat+=step)for(let lon=-180;lon<180;lon+=step){const p=project(lat,lon);if(p.z<-.04)continue;const q=screen(p,cx,cy,r);ctx.fillStyle=`rgba(91,167,255,${.07+.2*Math.max(0,p.z)})`;ctx.beginPath();ctx.arc(q.x,q.y,Math.max(.65,1.15*p.z),0,Math.PI*2);ctx.fill();}
  ...
  canvas.addEventListener('pointermove',e=>{lastInteraction=performance.now();const p=pos(e);pointerX=(p.x/w-.5)*2;pointerY=(p.y/h-.5)*2;if(drag){const dx=e.clientX-lastX,dy=e.clientY-lastY;vx=dx*.0007;vy=dy*.0007;targetY+=dx*.008;targetX=clamp(targetX+dy*.008,-1.15,1.15);lastX=e.clientX;lastY=e.clientY;}hover=locate(p.x,p.y);},{passive:true});
  const release=e=>{drag=false;lastInteraction=performance.now();try{canvas.releasePointerCapture(e.pointerId);}catch(_){} };
  canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('pointerleave',()=>{if(!drag){hover=-1;pointerX=0;pointerY=0;}},{passive:true});
